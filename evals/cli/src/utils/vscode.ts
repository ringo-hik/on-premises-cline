...
`,
	...
	const settings = {
		...
		"workbench.view.extension.khm.nyf.cline-for-on-premises-ActivityBar.visible": true,
	...
	const keybindings = [
		{
			key: "alt+c",
			command: "workbench.view.extension.khm.nyf.cline-for-on-premises-ActivityBar",
			when: "viewContainer.workbench.view.extension.khm.nyf.cline-for-on-premises-ActivityBar.enabled",
		},
		{
			key: "alt+shift+c",
	...
	const args = [
		...
		"--start-up-extension",
		"khm.nyf.cline-for-on-premises",
		// Run a command on startup to open Cline
		"--command",
		"workbench.view.extension.khm.nyf.cline-for-on-premises-ActivityBar",
	...
	const startupScript = `
		...
		setTimeout(() => {
			...
			require('vscode').commands.executeCommand('workbench.view.extension.khm.nyf.cline-for-on-premises-ActivityBar');
			...
			function activate(context) {
				...
				let disposable = vscode.commands.registerCommand('cline-activator.activate', async function () {
					try {
						...
						const extension = vscode.extensions.getExtension('khm.nyf.cline-for-on-premises');
						if (!extension) {
						...
						await vscode.commands.executeCommand('workbench.view.extension.khm.nyf.cline-for-on-premises-ActivityBar');
